...
rollout records, or checkpoints, bind its plan, the intended initial synthesis and
scoring configs, and the canonical training plan into a single preregistration:

```bash
python3 scripts/train_math500.py preregister-experiment \
  --output outputs/experiment/math500-preregistration.json \
  --scoring-config configs/evals/math500_scoring.toml \
...
step-1,000 selection. The registered scoring config fixes the
`sha256_uniform_per_question_v1` raw selector and seed before results exist. The
registered initial synthesis config must use
...
`score-synthesis` makes no model calls. It selects one of the eight existing
`pi_T` attempts per problem with the fixed selector in the scoring config and grades
only that attempt and its synthesized answer. No prior `score-raw` pass is required.
See [`docs/evaluation.md`](evaluation.md) for metric and artifact details. The paired
scores measure the trained policy's selected raw baseline and the additional gain,
if any, from frozen-`pi_0` synthesis over `pi_T` rollouts.
